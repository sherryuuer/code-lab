...
# test._Test__bar() 调用了 test 实例的私有方法 __bar。
# 在Python中，私有属性和方法的名称实际上是被重命名的（名字改编，name mangling），规则是 _类名__属性名。
# 所以 __bar 方法实际上被改名为 _Test__bar。
# 这就是为什么可以通过 test._Test__bar() 访问它。
```
